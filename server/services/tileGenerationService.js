...
async function generateNextHorizontalTile(previousTilePath, position) {
  ...
  try {
    ...
    const newTilePath = await queueApiRequest(async () => {
      ...
      const curlCommand = `curl -X POST "https://api.ideogram.ai/reframe" -H "Api-Key: ${config.IDEOGRAM_API_KEY}" -F "image_file=@${expandedImagePath}" -F "resolution=RESOLUTION_1408_704" -F "model=${config.IDEOGRAM_MODEL}" -F "prompt=${escapedPrompt}" -o "${responseFilePath}"`;
...
async function generateNextVerticalTile(bottomTilePath, position) {
  ...
  try {
    ...
    const newTilePath = await queueApiRequest(async () => {
      ...
      // Build the cURL command - escape quotes in the prompt
      const escapedPrompt = prompt.replace(/"/g, '\\"');
      const curlCommand = `curl -X POST "https://api.ideogram.ai/reframe" -H "Api-Key: ${config.IDEOGRAM_API_KEY}" -F "image_file=@${expandedImagePath}" -F "resolution=RESOLUTION_704_1408" -F "model=${config.IDEOGRAM_MODEL}" -F "prompt=${escapedPrompt}" -o "${responseFilePath}"`;
