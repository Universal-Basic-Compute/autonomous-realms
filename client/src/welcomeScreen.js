...
async function updateUserCompute(walletAddress, amount) {
  try {
    ...
    const response = await fetch(`${serverUrl}/api/auth/update-compute`, {
      method: 'POST',
      headers: {
...
async function createKinOSKin(kinName, leaderName, colonyName, dream, appearance) {
  try {
    const response = await fetch(`${serverUrl}/api/kinos/kins`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
`;
    ...
    try {
      ...
      const ttsResponse = await fetch(`${serverUrl}/api/kinos/tts?format=mp3`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    try {
      ...
      const imageResponse = await fetch(`${serverUrl}/api/kinos/kins/${kinName}/images`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
