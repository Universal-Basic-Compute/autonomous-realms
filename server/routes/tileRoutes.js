...
router.get('/curl-test', async (req, res) => {
  try {
    ...
    const curlCommand = `curl -X POST "https://api.ideogram.ai/reframe" -H "Api-Key: ${config.IDEOGRAM_API_KEY}" -F "image_file=@${testImagePath}" -F "resolution=RESOLUTION_1536_768" -F "model=${config.IDEOGRAM_MODEL}"`;
...
router.get('/api-test', async (req, res) => {
  try {
    ...
    formData.append('resolution', 'RESOLUTION_1536_768'); // For testing horizontal expansion
